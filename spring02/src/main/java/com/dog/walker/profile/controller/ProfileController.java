...
public class ProfileController {
	...
	private WalkerProfileService wpService;
	   ...
	   public ModelAndView insert(MultipartHttpServletRequest request,HttpSession session,@ModelAttribute ManageDto dto) {
	      // 서비스에 전달할 ModelAndView 객체 생성
	      ModelAndView mView = new ModelAndView();
	      // 서비스에 ModelAndVie 객체와 폼 전송된 회원 가입정보가
	      // 담겨있는 UsersDto 객체를 전달한다.
	      wpService.profileinsert(request, session, mView, dto);
